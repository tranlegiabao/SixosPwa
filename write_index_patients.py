                    ...
                    @foreach(var f in group.Facilities)
                    {
                        <div class="col-12">
                            <div class="border rounded-3 bg-light overflow-hidden">
                                <div class="p-3 d-flex justify-content-between align-items-center" style="cursor: pointer;" onclick="togglePatients('@f.Id')">
                                    <span class="fw-semibold text-dark text-truncate me-2" style="font-size: 15px;" title="@f.TenCoSo">@f.TenCoSo</span>
                                    <div>
                                        <span class="badge bg-success rounded-pill me-2">@f.PatientCount BN</span>
                                        <i class="fas fa-chevron-down text-muted" id="icon-@f.Id"></i>
                                    </div>
                                </div>
                                
                                <div id="patients-@f.Id" style="display: none;" class="bg-white border-top p-3">
                                    @if(f.PatientAccounts == null || !f.PatientAccounts.Any())
                                    {
                                        <div class="text-muted small">Cơ sở này chưa có tài khoản bệnh nhân nào.</div>
                                    }
                                    else
                                    {
                                        <div class="table-responsive">
                                            <table class="table table-sm table-hover mb-0">
                                                ...
                                                <tbody>
                                                    @foreach(var p in f.PatientAccounts)
                                                    {
                                                        <tr>
                                                            <td>#@p.Id</td>
                                                            <td>@p.SDT</td>
                                                            <td>@(string.IsNullOrEmpty(p.CCCD) ? "-" : p.CCCD)</td>
                                                        </tr>
                                                    }
                                                </tbody>
                                            </table>
...
<script>
    ...
    function togglePatients(facilityId) {
        var el = document.getElementById('patients-' + facilityId);
        var icon = document.getElementById('icon-' + facilityId);
        if (el) {
