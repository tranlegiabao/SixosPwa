...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const formFilter = document.getElementById('benhNhanFilterForm');
    if (formFilter) {
...
async function luuThongTinModal() {
    ...
    try {
        const resp = await fetch('/Admin/BenhNhan/CapNhatHoSo', {
            method: 'POST',
...
async function goNoiModal() {
    ...
    try {
        const resp = await fetch('/Admin/BenhNhan/GoNoiHoSo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function xoaThongTinHoSo(id, tenBN, accountId) {
    ...
    try {
        const resp = await fetch('/Admin/BenhNhan/XoaHoSo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function taoHoSoMoi(accountId) {
    ...
    try {
        const resp = await fetch('/Admin/BenhNhan/TaoHoSo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        tbody.innerHTML = `
            <tr class="empty-hoso-tr" id="empty-row-${accountId}">
                <td colspan="7" class="text-center py-4 text-muted">
                    <div>📋</div>
                    <div>Chưa có hồ sơ bệnh nhân nào.</div>
                </td>
            </tr>
        `;
    }
...
function capNhatThanhPhanTrang(page, maxPages, totalItems, daLoc, pageSize) {
    ...
    const hintLabel = document.getElementById('benhNhanHintLabel');
    ...
    if (!daLoc) {
        ...
        if (countLabel) countLabel.innerHTML = 'Vui lòng chọn cơ sở (hoặc nhập tiêu chí tìm kiếm) và bấm <strong>Lọc</strong> để xem danh sách bệnh nhân';
        if (hintLabel) hintLabel.innerText = '';
        return;
    }

    if (countLabel) {
        countLabel.innerHTML = totalItems > 0 ? `<strong>${totalItems.toLocaleString('vi-VN')}</strong> nhóm số điện thoại` : 'Không tìm thấy bệnh nhân nào phù hợp';
    }
    if (hintLabel) {
    ...
    try {
        ...
        if (resp.ok) {
            ...
            if (tbody) {
                ...
                khoiTaoDatePicker('#benhNhanTableBody');
            }
            ...
            showToast('Lỗi tải danh sách bệnh nhân từ máy chủ.', 'error');
        }
    } catch (err) {
